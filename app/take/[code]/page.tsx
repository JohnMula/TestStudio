    ...
    <Shell code={test.code}>
      {stage === 'intro' ? (
        <section className="flex flex-col gap-6 rounded-[16px] border border-border bg-card p-5 shadow-soft sm:p-8">
          <div className="flex flex-col gap-2"><span className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">Test ticket</span><h1 className="font-heading text-xl font-semibold tracking-tight text-foreground text-balance sm:text-2xl">{test.title}</h1><p className="text-sm text-muted-foreground">{orderedQuestions.length} {orderedQuestions.length === 1 ? 'question' : 'questions'} · {total} {total === 1 ? 'point' : 'points'} · {test.timeLimit === 'Off' ? 'No time limit' : `Time limit ${test.timeLimit} (auto-submits when it runs out)`} · No account needed.</p></div>
